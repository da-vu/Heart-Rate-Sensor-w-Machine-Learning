...
class ML:
    ...
    def train_hr_model(self, directory):
        ...
        for sub_id in unique_ids:
            ...
            for trial in range(len(sub_files)):#each file in the list of files for this subject
                ...
                while file_name == []:
                    print("bro it empty")
                    file_name = [os.path.basename(x) for x in glob.glob(directory+str(sub_id)+'_'+list_trials[trial+i]+'*.csv')]
                    i = i+1
                print(file_name)
                data = Data()
                try:
        ...
        arrr = np.reshape(arrr,(len(list_sub),2))
    ...
    def test_hr_model(self,directory):
        ...
        arr= np.reshape(arr,(len(unique_trial),2))
